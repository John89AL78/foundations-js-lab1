...
exports.numDays = function() {
  ...
  var mealsPerDay = [ 5, 4, 3, 6, 2, 4, 3, 4, 5, 1 ],
    tooHungryDay, sumMealsPerDay;
  
  for(var i = 0; i < mealsPerDay.length; i++) {
  	if(i === 0) {
  	  sumMealsPerDay = mealsPerDay[i];
  	} else {
  		sumMealsPerDay += mealsPerDay[i];
  	}

  	var averageMealsPerDay = sumMealsPerDay / (i + 1);
  	console.log(averageMealsPerDay);

  	if(averageMealsPerDay < 4.0) {
      tooHungryDay = i + 1;
  	}
  }
  return tooHungryDay;
